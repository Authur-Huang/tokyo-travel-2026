...
const prepData = [
    ...
    },
    {
        ...
        tone: "normal",
        items: [
            { id: "prep-vjw", title: "✅ Visit Japan Web 四人已完成註冊", desc: "**2026 年起入境審查與海關申報的 QR Code 已合併為「二合一 QR Code」，每人只有一個，不是兩組。**<br><br>**驗收標準：相簿裡要有 4 張 QR 截圖，不是 1 張。** 掛在同行家人下的孩子也各自有一個，要切換成員名字逐一截圖。<br><br>**⚠️ 一張 QR 只代表一個人。** 入境審查一定是一人一次（各自按指紋、拍臉）；海關電子申告也是各自建 QR、各自操作端末、各自過閘門。**唯一例外是身高未滿 100cm 的幼童**，可由代表人在「同伴家族」欄一併申報。<br><br>**建議四張 QR 分別存到各自手機**，四人可並排同時過關，不必擠在一起傳手機；也是防手機沒電的備援。<br><br>**另注意：** VJW 沒有官方 App，只能用瀏覽器（App Store 上同名的都是假的）。「免稅QR碼」是另一條線，需要入境時蓋在護照上的「上陸許可」證明章才能建立，**落地後才做，不是行前工作**。" },
            { id: "prep-sim", title: "eSIM ×4 下單（已改為全部用 eSIM）", desc: "**絕對不要合租一台分享器** —— Day 2/3/4 有媽媽與孩子分流路線，分開就失聯，四人必須各自獨立上網。<br><br>**已改為四人全用 eSIM**（免宅配、無下單死線）。<br><br>**⚠️ 先驗手機支不支援：** 撥 `*#06#` 看有沒有 **EID**，或 iPhone→設定→一般→關於本機看「可用的 eSIM」、Android→關於手機找 EID。**四支都要驗，特別是孩子的舊機** ─ 不支援就只能買實體卡，而實體卡要宅配時間。<br><br>**建議配置：** 爸媽用**吃到飽**（要當孩子的救援熱點）、孩子用 **10GB 總量型**（LINE＋位置分享＋拍照，5 天綽綽有餘，價差常一半以上）。線路選 docomo 或 SoftBank 原生皆可，全程在東京市區沒差別。<br><br>**三個必踩的雷：**<br>① **QR Code 只能掃一次，掃錯手機就報銷** ─ 四人份先標清楚誰是誰<br>② **安裝要在台灣有 Wi-Fi 時做完**（掃 QR 需要網路，落地沒網路就裝不了），**但別提前啟用** ─ 安全做法是 8/10 在家裝好、把該 eSIM 行動數據關閉，落地成田再打開<br>③ **吃到飽不是真的無限** ─ 幾乎都有 FUP（常見每日 3GB 高速額度，超過降速隔天恢復）。爸媽開熱點給兩個孩子會燒很快，這正是孩子要有獨立方案的原因" },
