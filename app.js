        ...
        activities: [
            {
                ...
                desc: "捷星日本 GK012 於 07:00 順利降落成田機場第三航廈。帶孩子辦理入境與行李提領。隨後前往第二航廈購買 Welcome Suica 卡，並買好京成電鐵 Skyliner 車票直奔秋葉原。首日因清晨剛落地未入住，早餐可在機場或抵達飯店後享用，其餘天數早餐皆在飯店內享用。<br><br>**⚠️ 入境時 Visit Japan Web 是一人一個 QR、各自過關**，沒手機的孩子用**紙本 QR**，出關前先確認他手上拿著。<br><br>**⚠️ Suica 別買錯張數：**沒手機的孩子沒有 Apple Pay，**一定要幫他買一張實體 Welcome Suica 並現金加值 ¥3,000**；其餘三人用手機綁定即可。",
...
const prepData = [
    ...
    },
    {
        ...
        items: [
            ...
            { id: "prep-sim", title: "eSIM ×3 下單（四個人只有三支手機）", desc: "**⚠️ 數量已修正為 3 張，不是 4 張。** 爸、媽、其中一個孩子有手機，**另一個孩子沒有手機且不另外補機**。<br><br>**絕對不要合租一台分享器** —— Day 2/3/4 有分流路線（媽媽單獨逛街、爸爸帶兩個孩子），分開就失聯，兩組都必須能獨立上網。<br><br>**⚠️ 先驗手機支不支援：** 撥 `*#06#` 看有沒有 **EID**，或 iPhone→設定→一般→關於本機看「可用的 eSIM」、Android→關於手機找 EID。**三支都要驗，特別是孩子的舊機** ─ 不支援就只能買實體卡，而實體卡要宅配時間。<br><br>**建議配置：** 爸媽用**吃到飽**、有手機的孩子用 **10GB 總量型**（LINE＋位置分享＋拍照，5 天綽綽有餘，價差常一半以上）。線路選 docomo 或 SoftBank 原生皆可，全程在東京市區沒差別。<br><br>**媽媽那支是單點故障：** 分流時媽媽那組只有一支手機，沒電或掉了就完全聯絡不上。**媽媽的行動電源要單獨帶一顆，不要跟爸爸共用。**<br><br>**三個必踩的雷：**<br>① **QR Code 只能掃一次，掃錯手機就報銷** ─ 三人份先標清楚誰是誰<br>② **安裝要在台灣有 Wi-Fi 時做完**（掃 QR 需要網路，落地沒網路就裝不了），**但別提前啟用** ─ 安全做法是 8/10 在家裝好、把該 eSIM 行動數據關閉，落地成田再打開<br>③ **吃到飽不是真的無限** ─ 幾乎都有 FUP（常見每日 3GB 高速額度，超過降速隔天恢復）" },
    ...
    },
    {
        ...
        items: [
            ...
            { id: "prep-carryon", title: "隨身包最終確認", desc: "護照、現金、手機、行動電源、**薄外套（機上冷氣很冷）**、頸枕、耳塞。液體一律 100ml 以下裝夾鏈袋。<br><br>**沒手機的孩子多兩樣：隨身紙卡 ×2、紙本 VJW QR ×1**，出門前當面檢查一次。" },
        ...
        if (act.momDesc && act.kidsDesc) {
            bodyHtml = `
                ...
                <div class="phone-rule">
                    <i class="fas fa-mobile-screen-button"></i> <strong>手機編組（全程不變）：</strong>四個人只有三支手機 ─ 媽媽自己一支，爸爸那組兩支。<strong>沒手機的孩子固定跟爸爸，不可與爸爸分開、不可自己回飯店</strong>；兩個孩子也不能單獨成一組。走散就站在原地不動，十分鐘沒人來再拿隨身紙卡找店員。
                </div>
            `;
        } else {
...
function calculateBudget() {
    ...
    if (foodPerPersonPerDay < 550) {
        ...
        advice += "💡 網路預算偏高。你們只有三支手機，而且 5 天用不到吃到飽 ─ 飯店有 WiFi，日間主要用途是導航與 LINE，每支 3GB 的總量型 eSIM 就很夠，3 張總計抓 NT$450-900 即可。";
