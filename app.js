...
const prepData = [
    {
        ...
        tone: "urgent",
        items: [
            { id: "prep-sim", title: "🔴 上網方案：eSIM ×2 ＋ 實體上網卡 ×1（實體卡今天就要下單）", desc: "**已確認兩件事：① 四個人只有三支手機**（爸、媽、其中一個孩子；另一個孩子沒有且不補機）；**② 有手機的那個孩子，手機不支援 eSIM** ─ 他只能用實體上網卡。<br><br>**所以要買：爸媽 2 張 eSIM ＋ 孩子 1 張實體卡。**<br><br>**🔴 為什麼這項被移到最緊急階段：**<br>eSIM 隨買隨掃，拖到出發前一晚都還來得及；**實體卡要宅配，時間邏輯完全不同**。你們 **8/10 晚上就出門**，中間卡著 8/8、8/9 週末。**最晚 8/7（五）下單並選超商取貨**才穩；拖到 8/8 才下單很可能 8/11 才到貨，那時人已經在日本了。<br><br>**⚠️ 已經過 8/7 才看到這則的話：不要再等宅配，直接走下面的成田備援。**<br><br>**備援（來不及就走這條，不要慌）：** Day 1 在**成田機場買**。T1、T2 入境大廳有 **SIM 卡自動販賣機（24 小時）**與電信櫃台，你們本來就要從 T3 走到 T2 買 Suica，順路。價格貴一點但一定買得到。<br><br>**⚠️ 不要賭「用爸爸手機開熱點就好」** ─ 分流時熱點跟著爸爸走，孩子那支就斷了。<br><br>**實體卡四個細節：**<br>① 買 **nano-SIM** 尺寸，下單前看商品頁<br>② **取卡針放爸爸隨身包**，不可託運 ─ 落地要馬上換卡<br>③ **拔下的台灣 SIM 收進固定小夾鏈袋跟護照放一起** ─ 掉在飯店地毯上就找不到了<br>④ **換卡後台灣門號收不到簡訊**（含銀行 OTP）─ 刷卡出國報備、訂位確認都要在換卡前做完<br><br>**卡到貨當天先插一次**：台灣會顯示「無服務」是正常的，但能確認尺寸對、卡槽讀得到、不是瑕疵品。**Android 可能要手動設 APN**，趁有網路先把設定步驟截圖。<br><br>**eSIM 兩張的注意事項：**<br>① **QR Code 只能掃一次，掃錯手機就報銷** ─ 先標清楚哪張是爸爸、哪張是媽媽<br>② **安裝要在台灣有 Wi-Fi 時做完**，但別提前啟用 ─ 8/10 在家裝好、行動數據先關掉，落地成田再打開<br><br>**媽媽那支是單點故障：** 分流時媽媽那組只有一支手機，沒電或掉了就完全聯絡不上。**媽媽的行動電源要單獨帶一顆，不要跟爸爸共用。**<br><br>**流量：** 每份抓 3GB 總量型就夠（導航＋LINE＋位置分享，一天約 300–500MB，飯店有 WiFi）。" },
    ...
    },
    {
        ...
        items: [
            ...
            { id: "prep-cash", title: "換日幣 ¥80,000 ~ ¥100,000（已換 ¥30,000，尚缺 ¥50,000~70,000）", desc: "銀行換匯比機場划算。**分兩批換、分兩個包放**，爸媽各帶一半。<br><br>**🔴 銀行只剩兩個營業日：8/7(五)、8/10(一)** ─ 8/8、8/9 是週末關門，而 8/10 已是出發當天（晚上就要出門）。<br><br>**大額現鈔多數分行要預約、隔 1~2 個工作日**，所以**8/7 一定要去問**，才留得下 8/10 取鈔的空間。**8/7 沒去 = 只剩 8/10 一天，而且是當天問當天要領，多數分行做不到。**<br><br>**⚠️ 真的來不及的備援：**① 桃園機場出境大廳的銀行櫃檯（**但你們是凌晨 00:40 到機場，櫃檯已關**，不可行）；② **成田機場入境後換匯**，匯率較差但金額有限、損失可接受；③ 日本 7-11 的 ATM 可用金融卡跨國提領（先確認卡片有開通國際提款與密碼為 4 碼）。" },
